...
//     event: Event;
//     type: 范_Echarts_事件之名称_Echarts_5_之实例;
// };

// type 范_wlcEcharts可成批绑定之事件处理程序 = (事件记载: 范_Echarts5_Echarts实例事件之记载) => any;
type 范_wlcEcharts可成批绑定之事件处理程序 = (事件记载: any) => any;

type 范_wlcEcharts可成批绑定之事件处理程序集 = { [事件名称: string]: 范_wlcEcharts可成批绑定之事件处理程序 };
...
type 范_Echarts部件之事件记录 = {
    ...
    本事件为同种事件中的第几次: number;
    此种Echarts事件在Echarts5中已弃用: boolean;
    此种事件为Zrender事件: boolean;

    事件记载?: any; // 范_Echarts5_Echarts实例事件之记载 | any;
};
...
export default class Page示范页1_Echarts折线图 extends Vue {
    private eCharts模块导出之根: typeof echarts = echarts
    private echarts之配置项集: EChartsOption = echarts之配置项集

    private 在界面列示的事件记录数目至多为 = 16
    private 应故意启用Echarts4的事件 = false
    private 应启用Echarts的rendered的事件 = true
    private 应启用Zrender的事件 = true
    private 应在浏览器控制打印除Rendered外之一切事件之记录 = true

    private eCharts实例各种事件之计数 = this.准备ECharts实例各种事件之计数()
    private wlcEcharts可成批绑定之事件处理程序集 = this.构建可成批对接的Echarts实例之事件之处理程序之配置项集()
    private 最末的若干条事件记载之列表: 范_Echarts部件之事件记录[] = []





    private get eChartsRendered事件发生之总次数 (): number {
        return this.eCharts实例各种事件之计数['rendered']
    }

    private get WlcEcharts事件记录区标题之措辞 (): string {
    ...
    private get WlcEcharts事件记录区副标题之措辞 (): string {
    ...
    private 准备ECharts实例各种事件之计数 (): 范_Echarts实例之事件计数对象 {
        return [
            ...SUPPORTED_ECHARTS_INSTANCE_EVENT_NAMES__ALL,
            ...SUPPORTED_ZRENDER_EVENT_NAMES__FROM_VUE_COMPONENT,
        ].reduce(
            ...
            {} as 范_Echarts实例之事件计数对象
        )
    }

    private 构建可成批对接的Echarts实例之事件之处理程序之配置项集 (): null | 范_wlcEcharts可成批绑定之事件处理程序集 {
        ...
        const wlcEcharts可成批绑定之事件处理程序集: 范_wlcEcharts可成批绑定之事件处理程序集 = {}
    ...
    private 为某种Echarts实例事件或某种Zrender事件构造一个处理程序 (
        事件名称: 范_Echarts_事件之名称_Echarts_任何版本之实例 | 范_Zrender_事件穿透本部件后之名称
    ): 范_wlcEcharts可成批绑定之事件处理程序 {
        const 此种事件之处理程序 = (事件记载: any): void => {
            const { eCharts实例各种事件之计数 } = this

            eCharts实例各种事件之计数[事件名称]++

            const 此种事件为Zrender事件 = SUPPORTED_ZRENDER_EVENT_NAMES__FROM_VUE_COMPONENT.includes(
                事件名称 as 范_Zrender_事件穿透本部件后之名称
            )

            const 此种Echarts事件在Echarts5中已弃用 = !此种事件为Zrender事件 && !SUPPORTED_ECHARTS_INSTANCE_EVENT_NAMES__ECHARTS_5.includes(
                事件名称 as 范_Echarts_事件之名称_Echarts_5_之实例
            )
    ...
    private 追加事件记录 (欲追加之事件记录: 范_Echarts部件之事件记录): void {
        const {
            事件名称,
            事件记载,
            本事件为同种事件中的第几次,
